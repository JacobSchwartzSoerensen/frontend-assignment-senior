...
export const fetchMockUsers = async ({perPage = 10, page = 0, searchTerm}: UserApiParams): Promise<User[]> => {
  const filteredUsers = mockUsers.filter(user => !searchTerm || user.login.startsWith(searchTerm));
  
  const pageStart = page * perPage;
  const pageEnd = pageStart + perPage;
  let userPage = filteredUsers.slice(pageStart, pageEnd);
